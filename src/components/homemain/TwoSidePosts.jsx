    ...
    <div className="row mb-5">
      {allPosts.map((post) => (
        <div key={post.id} className="col-md-4 mb-4">
          <div className="row g-0 post-card shadow-sm h-100 position-relative">
            <div className="col p-3 d-flex flex-column position-static">
              <strong className={`d-inline-block mb-1 ${post.categoryColor}`}>
                {post.category}
              </strong>
              ...
              <a
                href={post.url}
                className="stretched-link text-decoration-none small"
              >
                Continue reading →
              </a>
            </div>
